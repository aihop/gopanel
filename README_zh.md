...
> **说明**：当前仓库为 GoPanel **v1.0** 版本，基于 1Panel 二次开发。**v2.0** 正在全面重构中，功能持续演进，敬请期待。

<div align="center">

## 安全、好用的开发者面板

开发者专属，深度整合 Podman Rootless 技术，在非 Root 环境下构建坚实的权限护城河。我们重新定义了运维体验，让开发、测试到部署，皆可一键触达。

[快速安装](https://gopanel.run) · 在线演示 · [GitHub](https://github.com/aihop/gopanel)

</div>

### 更安全

- **基于 Podman Rootless**：默认以非 Root 权限运行服务与容器，把风险锁在"权限边界"内，生产环境更放心。
- 非 Root 权限执行，避免"一次漏洞 = 整机沦陷"的高危链路
- 更清晰的隔离与最小权限思路，减少误操作带来的系统级风险
- 默认安全基线更高，适合长期跑在公网的业务环境

### 更好用

- 由「胡说代码的可乐」主导开发，20 年研发/运维经验沉淀，把真实场景的高频操作做成一键。
- 自动 SSL 证书签发与续期，域名与站点上线更省心
- 自动网站与反向代理管理，常见场景开箱即用
- 流水线作业（CI/CD）+ 常用数据库可视化管理，日常维护更顺手

GoPanel 是一款由 Golang 开发、面向开发者与独立团队的现代化轻量级服务器管理面板，也是一个以安全为优先、基于 Podman rootless 的应用运行与网站部署平台。它不追求"大而全"的功能堆砌，而是把高频、关键的运维与交付动作收敛成一套更清爽、更快、更好理解的工作流。
...
- **Podman rootless 运行模式**：数据库、中间件、业务服务与网站运行环境通过 Podman 运行在无特权用户命名空间下，无需 root 守护进程即可实现容器隔离——迁移、备份、回滚和多环境管理更加安全可控。
- **默认安全的 rootless 架构**：不同于传统面板以 root 权限运行容器，GoPanel v2.0 彻底摒弃特权守护进程模式。每个容器运行在独立的无特权用户命名空间中，大幅降低攻击面，消除常见的权限提升风险。
...
- **数据库与容器管理**：在 Podman rootless 环境下统一查看运行状态、端口、版本、日志与资源占用；同时内置数据库管理能力，让数据库维护也能留在同一套控制台里完成——全程无需开放容器运行时的 root 权限。
