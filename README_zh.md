...
<p align="center">
  用手机下达开发任务、查看结构化进度、审查代码、审批高风险动作，并在同一个控制平面中管理产品上线所需的基础设施。
</p>
...
- **网站**：托管静态站、反向代理和容器化应用，管理域名、上游、访问日志和部署历史。
- **证书与 CDN**：申请和续签证书、管理 ACME 账号，并通过已配置的 CDN 规则推送证书。
- **容器**：管理 Docker 或 Podman 容器、Compose 项目、镜像、网络、存储卷、日志、端口和资源状态。
- **数据库**：安装数据库服务，并在面板中完成数据库、表、SQL、导入导出、迁移、备份和连接信息管理。
- **应用与进程**：安装常用服务、管理长期运行进程、查看主机资源、执行计划任务和维护备份。
