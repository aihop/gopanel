...
func (u *ComposeTemplateService) List() ([]dto.ComposeTemplateInfo, error) {
	composes, err := repo.NewIComposeTemplateRepo().List()
	if err != nil {
		return nil, errors.New("failed to get compose template list")
	}
...
func (u *ComposeTemplateService) Delete(ids []uint) error {
	...
	if len(ids) == 1 {
		...
		if compose.ID == 0 {
			return errors.New("compose template not found")
		}
...
func (u *ComposeTemplateService) Update(id uint, upMap map[string]interface{}) error {
	return repo.NewIComposeTemplateRepo().Update(id, upMap)
}
