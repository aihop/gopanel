...
func NewAppInstall() *AppInstallService {
	return &AppInstallService{db: global.DB, repo: repo.NewAppInstall()}
}
...
func (a *AppInstallService) GetInstallList() ([]dto.AppInstallInfo, error) {
	...
	for _, install := range appInstalls {
		datas = append(datas, dto.AppInstallInfo{ID: install.ID, Key: install.App.Key, Name: install.Name, HttpPort: install.HttpPort, HttpsPort: install.HttpsPort, ContainerName: install.ContainerName, RuntimeHost: install.RuntimeHost, RuntimeKind: install.RuntimeKind, RuntimeMode: install.RuntimeMode, RunUser: install.RunUser})
	}
	return datas, nil
}
...
func (s *AppInstallService) Update(appInstall *model.AppInstall) error {
	if err := s.db.Model(appInstall).Where("id = ?", appInstall.ID).Updates(appInstall).Error; err != nil {
		return err
	}
	return nil
}
func (s *AppInstallService) Delete(id uint) error {
	if err := s.db.Where("id = ?", id).Delete(&model.AppInstall{}).Error; err != nil {
		return err
	}
	return nil
}
...
func (s *AppInstallService) Uninstall(req AppUninstall) error {
	...
	if files.NewFileOp().Stat(composePath) {
		composeOperation := dto.ComposeOperation{Name: appInstall.Name, Path: composePath, Operation: "delete", WithFile: req.DeleteDir}
...
func (a *AppInstallService) Operate(req request.AppInstalledOperate) error {
	...
	case constant.Delete:
		if err := a.Uninstall(AppUninstall{ContainerName: install.ContainerName, DeleteDir: req.ForceDelete}); err != nil && !req.ForceDelete {
			return err
		}
		return nil
	case constant.Sync:
