...
func (u *BackupService) MysqlBackup(req *dto.CommonBackup, logger *BackupLogger) error {
	...
	record := &model.BackupRecord{Type: req.Type, Name: req.Name, DetailName: req.DetailName, Source: "LOCAL", BackupType: "LOCAL", FileDir: itemDir, FileName: fileName}
...
func handleMysqlBackup(serverId uint, dbName, targetDir, fileName string, logger *BackupLogger) error {
	...
	backupInfo := client.BackupInfo{Name: dbName, Type: resolveMysqlBackupType(dbInfo.Type, "mysql"), Version: version, Format: "sql.gz", TargetDir: targetDir, FileName: fileName, Timeout: calcMysqlBackupTimeout(estimatedBytes)}
	if logger != nil {
...
func handleMysqlRecover(req *dto.CommonRecover, isRollback bool, logger *BackupLogger) error {
	...
	if !isRollback {
		...
		if err := cli.Backup(client.BackupInfo{Name: req.DetailName, Type: dbType, Version: version, Format: "sql.gz", TargetDir: path.Dir(rollbackFile), FileName: path.Base(rollbackFile), Timeout: calcMysqlBackupTimeout(0)}); err != nil {
		...
		defer func() {
			if !isOk {
				...
				if err := cli.Recover(client.RecoverInfo{Name: req.DetailName, Type: dbType, Version: version, Format: "sql.gz", SourceFile: rollbackFile, Progress: buildMysqlRecoverProgressLogger("回滚中", logger), Timeout: calcMysqlRecoverTimeout(readFileSize(rollbackFile))}); err != nil {
	...
	if err := cli.Recover(client.RecoverInfo{Name: req.DetailName, Type: dbType, Version: version, Format: "sql.gz", SourceFile: req.File, Progress: buildMysqlRecoverProgressLogger("恢复中", logger), Timeout: recoverTimeout}); err != nil {
