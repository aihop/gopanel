...
func TestSyncAIProjectWorkspacePreservesUnmanagedFiles(t *testing.T) {
	...
	source := createAIProjectSourceDir(t, t.TempDir(), "api")
	...
	if _, err := syncAIProjectWorkspace(project, []string{source}); err != nil {
		t.Fatal(err)
	}
	manifest, err := readAIProjectWorkspaceManifest(workspaceDir)
	if err != nil {
	...
	if len(manifest.Sources) != 1 || manifest.Sources[0].LinkName != "api-2" {
		t.Fatalf("unexpected workspace sources: %#v", manifest.Sources)
	}
...
func TestSyncAIProjectWorkspaceReplacesManagedLink(t *testing.T) {
	withAIProjectBaseDir(t)
	sourcesRoot := t.TempDir()
	first := createAIProjectSourceDir(t, filepath.Join(sourcesRoot, "first"), "web")
	replacement := createAIProjectSourceDir(t, filepath.Join(sourcesRoot, "replacement"), "web")
	project := &model.AIGroup{ID: 8, CreatorID: 4}
	workspaceDir, err := syncAIProjectWorkspace(project, []string{first})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := syncAIProjectWorkspace(project, []string{replacement}); err != nil {
		t.Fatal(err)
	}
	target, err := filepath.EvalSymlinks(filepath.Join(workspaceDir, "web"))
