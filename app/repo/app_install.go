...
func NewAppInstall() *AppInstallRepo {
	return &AppInstallRepo{
		DB: global.DB,
	}
}

func (r *AppInstallRepo) MigrateTable() error {
	return r.DB.AutoMigrate(&model.AppInstall{})
}

func (r *AppInstallRepo) WithTx(tx *gorm.DB) *AppInstallRepo {
	if tx == nil {
		tx = global.DB
	}
	r.DB = tx
