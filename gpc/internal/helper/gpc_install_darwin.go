...
func (s *Server) actionGoPanelGPCInstall(ctx context.Context, params map[string]interface{}) (string, error) {
	...
	if err := copyExecutableFileDarwin(srcAbs, tmpPath); err != nil {
		_ = os.Remove(tmpPath)
		return "", err
	}
	...
	if err := scheduleDarwinServiceRestart(serviceName); err != nil {
		return "", err
	}
...
func copyExecutableFileDarwin(src, dst string) error {
	in, err := os.Open(src)
	if err != nil {
		return err
	}
