...
func (f *Ufw) RichRules(rule FireInfo, operation string) error {
	...
	addr := strings.TrimSpace(rule.Address)
	addrLower := strings.ToLower(strings.ReplaceAll(strings.ReplaceAll(addr, "(v6)", ""), "(v4)", ""))
	addrLower = strings.TrimSpace(addrLower)
	if addrLower == "anywhere" || addrLower == "0.0.0.0/0" || addrLower == "::/0" {
		addr = ""
	}
	rule.Address = addr
