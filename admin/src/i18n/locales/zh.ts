export default {
	...
	database: {
		...
		databaseAccountAccessTips: "MySQL 使用“用户名@Host”识别账户，localhost、% 和特定地址是独立账户；% 仅表示账号接受远程来源，仍需数据库监听、端口和防火墙放行。PostgreSQL 角色没有 Host 字段，远程来源由 listen_addresses、pg_hba.conf、端口和防火墙统一控制。密码未托管表示数据库无法反查原密码，可通过“修改”设置新密码后由面板托管。",
		addServer: "添加服务器",
		type: "类型",
		copy: "复制",
		privileges: "权限",
	...
	pipeline: {
		...
		forceDeleteDangerDescription: "此操作不可恢复。若流水线正在执行、已接入交付流程、存在交付历史或 Runner 仍被网站使用，系统仍会拒绝删除。",
