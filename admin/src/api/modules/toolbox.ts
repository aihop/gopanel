...
import type { ReqPage, ResPage, UpdateByFile } from "../interface"
import type { Toolbox } from "../interface/toolbox"
...
export const updateDevicePasswd = (user: string, passwd: string) => {
	return http.post(
		...
		{ user: user, passwd: enc.Base64.stringify(enc.Utf8.parse(passwd)) },
		TimeoutEnum.T_60S
	)
}
