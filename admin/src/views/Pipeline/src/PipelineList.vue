...
const buildPipelineRuntimeText = (
	row: Pipeline.ResPipeline | null,
	options?: { prefix?: string; runUserPrefix?: string }
) => {
	if (!row) return ""
...
const columns: DataTableColumns<Pipeline.ResPipeline> = [
	...
	{
		title: t("pipeline.mode"),
		key: "runnerMode",
		render: (row: Pipeline.ResPipeline) => {
			const isRunner = (row.runnerMode || "").toLowerCase() === "runner"
			return h(
				NTag,
				{ type: isRunner ? "success" : "warning", size: "small" },
				{
					default: () => (isRunner ? t("pipeline.modeRunner") : t("pipeline.modeScript"))
				}
			)
		}
	},
		...
		render: (row: Pipeline.ResPipeline) => {
			return h("div", { class: "flex flex-col gap-1" }, [
				h("div", { class: "flex flex-wrap items-center gap-2" }, [
					h(
						NTag,
						{ type: row.runtimeKind === "docker" ? "success" : "warning", size: "small" },
						{
					...
					h(
						NTag,
						{ type: row.runtimeMode === "rootless" ? "warning" : "default", size: "small" },
						{
							default: () =>
				...
				h("div", { class: "flex items-center gap-1 text-xs text-slate-500" }, [
					...
					row.runtimeHost
						? h(
								...
								{
									trigger: () =>
										h(Icon, { name: "mdi:server-outline", size: 15, class: "text-slate-500" }),
	...
	{
		title: t("pipeline.branch"),
		key: "branch",
		render: (row: Pipeline.ResPipeline) => h(NTag, { type: "info", size: "small" }, { default: () => row.branch })
	},
	{
		title: t("pipeline.currentVersion"),
		key: "version",
		render: (row: Pipeline.ResPipeline) =>
	...
	},
	{
		...
		key: "createdAt",
		render: (row: Pipeline.ResPipeline) =>
			h("div", { class: "text-xs text-slate-500" }, formatTime(row.createdAt || ""))
	},
	{
		...
		fixed: "right",
		render(row: Pipeline.ResPipeline) {
			const options = [{ key: "releases", label: t("pipeline.releases") }]
...
const confirmRun = async () => {
	...
	if (!runFormModel.value.version) {
		message.warning(t("pipeline.versionRequired"))
		return
	}
	...
	try {
		...
		message.success(t("pipeline.runTriggered", { version: runFormModel.value.version }))
...
const handleRetryFromLogs = async () => {
	...
	try {
		...
		message.success(t("pipeline.rerunTriggered", { version: runFormModel.value.version }))
...
<template>
	<div class="w-full">
		...
		<n-modal
			...
			preset="card"
			:title="t('pipeline.buildVersionTitle')"
			style="width: 400px"
			class="w-full !rounded-[24px] shadow-[0_24px_48px_rgba(15,23,42,0.12)] sm:w-[90%]"
		>
			<div v-if="currentRunRow" class="mb-4 text-sm text-slate-500">
				{{ t("pipeline.triggering") }}
				<span class="font-semibold text-blue-600">{{ currentRunRow.name }}</span>
				{{ t("pipeline.pipelineWord") }}
				<br />
				{{ t("pipeline.currentVersion") }}:
				<span class="font-mono font-semibold">v{{ currentRunRow.version }}</span>
			</div>
			<div
				v-if="currentRunRow"
			...
			<n-form :model="runFormModel" label-placement="left" label-width="80">
				<n-form-item :label="t('pipeline.buildVersion')">
					<n-input v-model:value="runFormModel.version" :placeholder="t('pipeline.versionPlaceholder')" />
				</n-form-item>
				<div class="mt-4 flex justify-end gap-3">
					<n-button @click="runModalShow = false">{{ t("commons.button.cancel") }}</n-button>
					<n-button type="primary" :loading="runLoading" @click="confirmRun">
						{{ t("pipeline.confirmRun") }}
					</n-button>
				</div>
			</n-form>
		</n-modal>
