...
export const useSSLManagement = () => {
	...
	const detailTitle = computed(() =>
		currentSSL.value
			? t("website.sslDetailTitle", { domain: currentSSL.value.primaryDomain })
			: t("website.sslDetailTitleDefault")
	)
	...
	const getCloudAccountLabel = (cloudAccountId: number) => {
		...
		return account ? `${account.name} (${account.type})` : t("website.unknownAccountId", { id: cloudAccountId })
	}
	async function handleCdnAccountChange(value: number | null) {
		...
		try {
			...
			if (!cdnDomainsOptions.value.length) {
				message.info(t("website.noCdnDomains"))
			}
		} catch (error: any) {
			void 0
		} finally {
	...
	async function handleSyncCertificate() {
		if (!syncForm.websiteId) {
			message.warning(t("website.selectWebsite"))
			return
		}
		submitting.value = true
		try {
			await ObtainSSL({ ID: syncForm.websiteId })
			message.success(t("website.syncedCaddyCert"))
	...
	async function handleRenewCertificate(row: SSLRow) {
		dialog.info({
			title: t("website.renewConfirmTitle"),
			content: t("website.renewConfirmContent", { domain: row.primaryDomain }),
			...
			onPositiveClick: async () => {
				...
				try {
					...
					message.success(t("website.renewSubmitted"))
					openLogModal(row.id)
				} catch (error: any) {
					void 0
				} finally {
	...
	async function handleCloudApplyCertificate() {
		if (!cloudApplyForm.primaryDomain || !cloudApplyForm.cloudAccountId) {
			message.warning(t("website.fillDomainAndCloudAccount"))
			return
		}
		submitting.value = true
		try {
			...
			message.success(t("website.cloudApplySubmitted"))
	...
	async function handleUploadCertificate() {
		if (!uploadForm.primaryDomain || !uploadForm.pem || !uploadForm.privateKey) {
			message.warning(t("website.fillDomainCertAndKey"))
			return
		}
		submitting.value = true
		try {
			...
			message.success(t("website.certUploaded"))
	...
	async function handleApplyCertificate() {
		...
		try {
			...
			message.success(t("website.certBoundToWebsite"))
	...
	async function handlePushCDN() {
		...
		try {
			...
			message.success(t("website.pushedToCdn"))
	...
	function confirmDelete(row: SSLRow) {
		dialog.warning({
			...
			positiveText: t("commons.button.confirm"),
			negativeText: t("commons.button.cancel"),
			content: row.type === "caddy" ? t("website.deleteCaddyCertHint") : t("website.deleteUploadedCertHint"),
			onPositiveClick: async () => {
