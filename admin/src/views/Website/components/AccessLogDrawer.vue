<template>
	...
	<n-drawer v-model:show="visible" :width="880" placement="right">
			...
			<div class="space-y-5">
				<n-card size="small" :title="t('securityMonitoring.websiteRiskSummary')">
					<n-spin :show="riskLoading">
						...
						<n-empty
							v-else-if="!websiteRisks.length"
							:description="t('securityMonitoring.websiteRiskEmpty')"
						/>
						<div v-else class="space-y-2">
							<n-alert
								v-for="risk in websiteRisks"
								:key="risk.id"
								:type="risk.level === 'critical' || risk.level === 'high' ? 'error' : 'warning'"
							>
								<div class="font-semibold">
									{{ t(`securityMonitoring.level.${risk.level}`) }} · {{ risk.summary }}
								</div>
				...
				<div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
					<div class="flex flex-wrap items-center justify-between gap-3">
						<div>
							...
							<div v-if="bindingRuntimeText" class="mt-2 text-xs text-slate-500">
								{{ bindingRuntimeText }}
							</div>
						</div>
						<div class="flex flex-wrap gap-2">
							<n-button v-if="!isErrorView" size="small" ghost @click="openTodayIPStats">
								{{ t("website.todayIPStats") }}
							</n-button>
							<n-button size="small" @click="loadLogs(false)">
								{{ t("commons.button.refresh") }}
							</n-button>
							<n-button size="small" type="primary" ghost @click="loadLogs(true)">
								{{ t("website.locateLatest") }}
							</n-button>
						</div>
					</div>
				</div>
				...
				<div class="flex items-center justify-between gap-3">
					<div class="text-xs text-slate-500">
						{{ end ? t("website.reachedLogEnd") : t("website.moreLogsAvailable") }}
					</div>
					<div class="flex gap-2">
						<n-button size="small" :disabled="page <= 1 || loading" @click="changePage(page - 1)">
							{{ t("website.prevPage") }}
						</n-button>
						<n-button size="small" :disabled="page >= total || loading" @click="changePage(page + 1)">
							{{ t("website.nextPage") }}
						</n-button>
					</div>
				</div>
...
const drawerTitle = computed(() =>
	logType.value === "error" ? t("website.websiteErrorLog") : t("website.websiteAccessLog")
)
const logLabel = computed(() => (logType.value === "error" ? t("website.errLog") : t("website.accessLogLabel")))
const emptyText = computed(() =>
	logType.value === "error" ? t("website.noErrorLogEmpty") : t("website.noAccessLogEmpty")
)
const logPanelTitle = computed(() => (logType.value === "error" ? "Error Log" : "Access Log"))
const simpleHelperText = computed(() =>
	logType.value === "error" ? t("website.errorLogHelper") : t("website.accessLogHelper")
)
const parsedEntries = computed(() => logLines.value.map(parseLogLine).filter((item): item is ParsedLogEntry => !!item))
const filteredEntries = computed(() =>
	parsedEntries.value
		.filter(item => matchStatusFilter(item, statusFilter.value))
		.filter(item => matchSearchKeyword(item, searchKeyword.value))
)
...
const detailTitle = computed(() =>
	!activeEntry.value ? t("website.logDetail") : `${activeEntry.value.method} ${activeEntry.value.path}`
)
...
async function copyPath() {
	...
	if (!path) {
		message.warning(t("website.noLogPath"))
		return
	}
	await copyText(path)
}
