...
function sessionProjectName(session: CodeSession, projects: AIProject[]) {
	...
	return (
		projects.find(project => {
			const projectPaths = [project.workDir, ...(project.sourceDirs || [])]
...
function sessionApprovalLabel(session: CodeSession) {
	return t(
		{ manual: "mobile.approvalManual", safe_auto: "mobile.approvalSafe", full_auto: "mobile.approvalFull" }[
	...
	<section>
		<div class="my-6 flex items-center justify-between">
			<div class="flex items-center gap-2">
				...
				<n-tag v-if="pendingCount" size="small" type="warning" :bordered="false">
					{{ t("mobile.pendingCount", { count: pendingCount }) }}
				</n-tag>
			</div>
			<n-button size="small" text type="primary" @click="emit('showAll')">
				{{ t("mobile.viewProjectSessions") }}
			</n-button>
		</div>
		...
		<div v-else class="space-y-3">
			<button
				...
				:key="session.id"
				class="w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition active:scale-[0.99]"
			...
			>
				<div class="flex items-start justify-between gap-3">
					<div class="min-w-0 flex-1">
						...
						<div class="mt-1.5 flex items-center gap-1.5 text-sm font-medium text-blue-600">
							...
							<span class="truncate">{{ sessionProjectName(session, projects) }}</span>
						</div>
				...
				<div
					class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-slate-100 pt-3 text-xs text-slate-400"
				>
